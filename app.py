...
def generate_automated_audit(cv_text, jd_text):
    # 1. Clean the key to remove accidental spaces or line breaks
    api_key = GEMINI_API_KEY.strip()
    
    if not api_key or api_key == "AQ.Ab8RN6Kk3_Wsro9iv0sW5S8r1q3oQs7GkoUWnOpOTkVk98M7qw":
        return "⚠️ **Missing API Key:** Please paste a valid key into `GEMINI_API_KEY` near the top of `app.py`."
    
    try:
        # 2. Configure genai directly inside the execution call
        genai.configure(api_key=api_key)
        
        # 3. Use gemini-1.5-flash (gemini-pro is deprecated)
        ...
        Keep your response structured with bullet points.
        """
...
with st.sidebar:
    ...
    st.info("Upload a candidate CV and paste the job description to run evaluation.")
